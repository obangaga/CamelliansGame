...
import { Sword, Wand2, Check, Sparkles, User, ArrowUp, ArrowDown, ArrowLeft, ArrowRight } from "lucide-react";
...
function buildWorld() {
  ...
  const road = (r, c) => {
    if (grid[r] && (grid[r][c] === "grass" || grid[r][c] === "tree")) setT(r, c, "road");
  };
  const lineH = (r, c1, c2) => {
    const [a, b] = c1 < c2 ? [c1, c2] : [c2, c1];
    for (let c = a; c <= b; c++) road(r, c);
  };
  const lineV = (c, r1, r2) => {
    const [a, b] = r1 < r2 ? [r1, r2] : [r2, r1];
    for (let r = a; r <= b; r++) road(r, c);
  };
...
const NPCS = [
  ...
  { r: 19, c: 20, name: "Penjaga", skin: "#C68A5B", hair: "#1C1B1F", outfit: "#4A5A7A", dialog: "Jembatan ini menghubungkan kita ke kota. Hati-hati, arusnya deras." },
...
function drawPerson(ctx, px, py, skin, hair, outfit, facing, bob, mirror) {
  ...
  ctx.beginPath();
  ctx.ellipse(px, py + 1, 6, 2, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;

  ctx.fillStyle = outfit;
  ctx.fillRect(px - 3 + legOffset, py - 7, 3, 7);
  ctx.fillRect(px + legOffset - legOffset * 2, py - 7, 3, 7);
...
function WorldScreen({ character, onExit }) {
  ...
  const playerRef = useRef({ x: 5 * TILE + 8, y: 19 * TILE + 8, facing: "down", moving: false });
  ...
  useEffect(() => {
    ...
    const down = (e) => {
      ...
      if (e.key === "Enter" || e.key === " ") {
        if (nearby) setDialog(nearby);
      }
    };
    const up = (e) => {
      const k = map[e.key];
      if (k) keysRef.current[k] = true && (keysRef.current[k] = false);
    };
    ...
    const loop = (now) => {
      ...
      for (let r = r0; r < r1; r++) {
        for (let c = c0; c < c1; c++) {
          ...
          const type = grid[r][c];
          drawTileBase(ctx, type, c * TILE - camX, r * TILE - camY, r, c, now);
        }
      }
      for (let r = r0; r < r1; r++) {
        for (let c = c0; c < c1; c++) {
          if (r < 0 || c < 0 || r >= ROWS || c >= COLS) continue;
          const type = grid[r][c];
          if (type === "tree" || type === "house" || type === "sign") {
            drawFeature(ctx, type, c * TILE - camX, r * TILE - camY);
          }
        }
      }
      ...
      NPCS.forEach((n) => {
        ...
        if (d < closestD) {
          closestD = d;
          closest = { type: "npc", name: n.name, dialog: n.dialog };
        }
      });
      SIGNS.forEach((s) => {
        const wx = s.c * TILE + 8;
        ...
        if (d < closestD) {
          closestD = d;
          closest = { type: "sign", name: "Papan Pengumuman", dialog: s.text };
        }
      });
      setNearby((prev) => {
        const same = prev && closest && prev.name === closest.name;
  ...
  const setKey = (dir, val) => (e) => {
    e.preventDefault();
    keysRef.current[dir] = val;
  };

  const ClassIcon = character.classAccent === CLASSES[0].accent ? Sword : Wand2;
    ...
    <div style={{ minHeight: "100vh", background: "#0B0D12", color: "#F4F5F9", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div
              ...
              style={{ width: 34, height: 34, background: `${character.classAccent}22`, border: `1px solid ${character.classAccent}` }}
            >
              <ClassIcon size={16} color={character.classAccent} />
            </div>
            <div>
              <p className="heading text-sm" style={{ fontWeight: 700 }}>{character.name}</p>
              <p className="text-xs" style={{ color: "#8B90A3" }}>{character.classLabel}</p>
            </div>
          </div>
          <button
            onClick={onExit}
            style={{ border: "1px solid #262A38", color: "#8B90A3" }}
        ...
        >
          ...
          {nearby && (
            <div
              className="absolute left-1/2 -translate-x-1/2 rounded-xl px-3 py-2 text-xs"
              style={{ top: 10, background: "#12141Ceb", border: "1px solid #262A38", maxWidth: "80%" }}
            >
              <p style={{ color: character.classAccent, fontWeight: 600 }} className="mb-0.5">{nearby.name}</p>
              <p style={{ color: "#C7CBD8" }}>{nearby.dialog}</p>
            </div>
          )}
        </div>

        <p className="text-center text-xs mt-3" style={{ color: "#5B6072" }}>
          Gerakkan dengan WASD atau tombol panah
        </p>
        ...
        <div className="flex justify-center mt-4 md:hidden">
          <div style={{ width: 132, height: 132, position: "relative" }}>
            {[
              ...
              <button
                key={dir}
                className="dpad-btn absolute rounded-xl flex items-center justify-center"
                style={{ width: 40, height: 40, background: "#161922", border: "1px solid #262A38", ...style }}
                onPointerDown={setKey(dir, true)}
                onPointerUp={setKey(dir, false)}
                onPointerLeave={setKey(dir, false)}
...
function CharacterPreview({ gender, hairHex, outfit, skinHex, classId }) {
  ...
  return (
    <svg viewBox="0 0 220 300" width="100%" height="100%" role="img" aria-label="Preview karakter">
      ...
      <path
        d="M70,72 C70,45 90,27 110,27 C130,27 150,45 150,72 C150,81 146,87 146,87 C146,87 140,59 110,59 C80,59 74,87 74,87 C74,87 70,81 70,72 Z"
        fill={hairHex}
      />
    </svg>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0B0D12", color: "#F4F5F9", fontFamily: "'Inter', sans-serif" }}>
      <style>{`
        ...
        input:focus-visible, button:focus-visible { outline: 2px solid #7C5CFC; outline-offset: 2px; }
        .swatch { transition: transform 0.15s ease; }
        .swatch:hover { transform: translateY(-2px); }
      `}</style>
      <div className="max-w-6xl mx-auto px-5 md:px-8 py-10 md:py-14">
        <div className="mb-10">
          <p className="uppercase text-xs mb-2" style={{ color: cls.accent, letterSpacing: "0.3em", fontWeight: 600 }}>
            Character Creator
          </p>
        ...
        <div className="grid md:grid-cols-[minmax(280px,380px)_1fr] gap-8">
          <div
            className="md:sticky md:top-8 self-start rounded-3xl p-6 flex flex-col items-center"
            style={{ background: "#12141C", border: "1px solid #21242F", height: "fit-content" }}
          >
            <div className="relative w-full flex items-center justify-center mb-5" style={{ height: 300 }}>
              <div
                className="absolute rounded-full"
                style={{ width: 220, height: 60, bottom: 6, background: `radial-gradient(ellipse, ${cls.accent}33 0%, transparent 70%)`, filter: "blur(6px)" }}
              />
          ...
          <div className="rounded-3xl p-6 md:p-8" style={{ background: "#12141C", border: "1px solid #21242F" }}>
            <section className="mb-8">
              ...
              <input
                ...
                maxLength={20}
                placeholder="Tulis nama karaktermu..."
                style={{ background: "#0D0F16", border: `1px solid ${error ? "#FF6B4A" : "#262A38"}`, color: "#F4F5F9" }}
            ...
            <section className="mb-8">
              ...
              <div className="flex gap-3 flex-wrap">
                {HAIR_COLORS.map((h) => {
                  ...
                  return (
                    <button key={h.id} onClick={() => setHairId(h.id)} aria-label={h.label} title={h.label} className="swatch rounded-full"
            ...
            <section className="mb-9">
              ...
              <div className="flex gap-3 flex-wrap">
                {SKIN_TONES.map((s) => {
                  ...
                  return (
                    <button key={s.id} onClick={() => setSkinId(s.id)} aria-label={s.id} className="swatch rounded-full"
            ...
            <button
              onClick={handleStart}
              style={{ background: `linear-gradient(135deg, ${cls.accent}, ${cls.accent}CC)`, color: "#0B0D12" }}
...
export default function App() {
  ...
  if (screen === "world" && character) {
    return <WorldScreen character={character} onExit={() => setScreen("create")} />;
  }
  return (
    <CreateScreen
      onStart={(c) => {
